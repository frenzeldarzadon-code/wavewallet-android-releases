...
BEGIN
  ...
  IF _scope_recipient IS NOT NULL AND _scope_recipient <> _uid THEN
    IF NOT _super THEN
      IF NOT EXISTS (
        ...
        WHERE p.id = _scope_recipient AND is_ecosystem_admin(_uid, p.ecosystem_id)
      ) AND NOT EXISTS (
        SELECT 1 FROM public.ecosystem_memberships m
        WHERE m.user_id = _scope_recipient AND is_ecosystem_admin(_uid, m.ecosystem_id)
      ) THEN RAISE EXCEPTION 'Not allowed to read these earnings'; END IF;
    END IF;
  ELSIF _scope_recipient IS NULL THEN
    IF _scope_ecosystem IS NULL THEN
      IF NOT _super THEN _scope_recipient := _uid; END IF;
    ELSE
  ...
  /*
   * SALE COMMISSIONS
   * Every completed voucher sale has 100% gross value = list_price x quantity.
   * Admin retained earnings are the remaining share after the applicable
   * reseller hierarchy. The commission rows themselves are the authoritative
   * historical rate snapshots; their amounts were reconciled once to the
   * hierarchy rule and are used as stored values here.
   */
  RETURN QUERY
  SELECT
    'sc:' || sc.id::text,
    sc.created_at,
    sc.ecosystem_id,
    CASE sc.kind WHEN 'upline' THEN 'upline_commission'
    ...
    vs.product_name,
    vs.quantity,
    ROUND(vs.list_price * GREATEST(COALESCE(vs.quantity,1),1),2),
    sc.credits_consumed,
    CASE
      WHEN sc.kind='admin' THEN ROUND(sc.commission_amount * 100.0 / NULLIF(vs.list_price * GREATEST(COALESCE(vs.quantity,1),1),0),2)
      ELSE sc.commission_percent::numeric
    END,
  ...
  /* Legacy sale commission credits without a sale_commissions row. */
  SELECT
    'cl:' || cl.id::text,
    cl.created_at,
    cl.ecosystem_id,
  ...
  WHERE cl.entry_kind='sale_commission' AND cl.direction='credit'
  ...
  /* Wholesale discount benefit for reseller/subreseller purchases. */
  SELECT
    'wd:' || vs.id::text,
    vs.created_at,
    vs.ecosystem_id,
  ...
  /*
   * Derived Admin earnings for sales that do not have an explicit admin
   * commission row.
   *
   * - Admin purchase (legacy, no commission row): 100% of gross.
   * - Reseller purchase: Admin = gross - reseller discount.
   * - Subreseller purchase: Admin = gross - subreseller discount - parent
   *   reseller's remaining cashback.
   * - Customer purchase: Admin = gross - all recorded reseller/upline cashback.
   */
  SELECT
    'am:' || vs.id::text,
    vs.created_at,
    ...
    ROUND(vs.list_price * GREATEST(COALESCE(vs.quantity,1),1),2),
    CASE
      WHEN vs.buyer_role='admin' THEN 100::numeric
      WHEN vs.list_price > 0 THEN ROUND((CASE
        WHEN vs.buyer_role='reseller' THEN ROUND((vs.list_price*GREATEST(COALESCE(vs.quantity,1),1))*(100-vs.discount_percent)/100.0,2)
        WHEN vs.buyer_role='subreseller' THEN ROUND((vs.list_price*GREATEST(COALESCE(vs.quantity,1),1))*(100-COALESCE(vs.discount_percent,0)-COALESCE(vs.upline_commission_percent,0))/100.0,2)
        ELSE ROUND((vs.list_price*GREATEST(COALESCE(vs.quantity,1),1)) - COALESCE((SELECT SUM(sc2.commission_amount) FROM public.sale_commissions sc2 WHERE sc2.sale_id=vs.id AND sc2.kind <> 'admin' AND sc2.reversed_at IS NULL),0),2)
      END) * 100.0 / (vs.list_price*GREATEST(COALESCE(vs.quantity,1),1)),2)
      ELSE 0 END,
    CASE
      WHEN vs.buyer_role='admin' THEN ROUND(vs.list_price*GREATEST(COALESCE(vs.quantity,1),1),2)
      WHEN vs.buyer_role='reseller' THEN ROUND((vs.list_price*GREATEST(COALESCE(vs.quantity,1),1))*(100-vs.discount_percent)/100.0,2)
      WHEN vs.buyer_role='subreseller' THEN ROUND((vs.list_price*GREATEST(COALESCE(vs.quantity,1),1))*(100-COALESCE(vs.discount_percent,0)-COALESCE(vs.upline_commission_percent,0))/100.0,2)
      ELSE ROUND((vs.list_price*GREATEST(COALESCE(vs.quantity,1),1)) - COALESCE((SELECT SUM(sc3.commission_amount) FROM public.sale_commissions sc3 WHERE sc3.sale_id=vs.id AND sc3.kind <> 'admin' AND sc3.reversed_at IS NULL),0),2)
    END,
    CASE WHEN vs.refunded_at IS NOT NULL THEN 'reversed' ELSE 'settled' END,
    vs.tx_id,
    vs.id
  ...
  LEFT JOIN LATERAL (
    SELECT x.id,x.full_name FROM (
      ...
      WHERE m.ecosystem_id=vs.ecosystem_id AND m.role='admin' AND m.membership_state='active' AND m.status='active'
      UNION ALL
      SELECT p.id,p.full_name,1,p.created_at
      FROM public.profiles p JOIN public.user_roles ur ON ur.user_id=p.id AND ur.role='admin'
      WHERE COALESCE(ur.ecosystem_id,p.ecosystem_id)=vs.ecosystem_id
    ) x ORDER BY x.pri,x.at LIMIT 1
  ) adm ON true
  ...
  WHERE vs.created_at >= _start AND vs.created_at <= _end
    ...
    -- One authoritative shop-retained earning per sale: the derived row exists
    -- only when the sale has no unreversed admin commission record. Admin
    -- self-purchases record a real admin commission, so they no longer produce
    -- a second 100%-of-face-value row on top of it.
    AND NOT EXISTS (SELECT 1 FROM public.sale_commissions sa WHERE sa.sale_id=vs.id AND sa.kind='admin' AND sa.reversed_at IS NULL)
    AND (_scope_recipient IS NULL OR adm.id=_scope_recipient)
    AND (_scope_ecosystem IS NULL OR vs.ecosystem_id=_scope_ecosystem)

  UNION ALL

  /* Approved credit-paid retail orders: full order value belongs to the shop. */
  SELECT
    'ro:' || ro.id::text,
  ...
  LEFT JOIN LATERAL (
    SELECT x.id,x.full_name FROM (
      SELECT p.id,p.full_name,0 AS pri,m.created_at AS at
      FROM public.ecosystem_memberships m JOIN public.profiles p ON p.id=m.user_id
      WHERE m.ecosystem_id=ro.ecosystem_id AND m.role='admin' AND m.membership_state='active' AND m.status='active'
      UNION ALL
      SELECT p.id,p.full_name,1,p.created_at
      FROM public.profiles p JOIN public.user_roles ur ON ur.user_id=p.id AND ur.role='admin'
  ...
  WHERE COALESCE(ro.decided_at,ro.created_at) >= _start AND COALESCE(ro.decided_at,ro.created_at) <= _end
    AND ro.status='approved' AND ro.payment_method='credit' AND ro.total>0 AND adm2.id IS NOT NULL
    AND (_scope_recipient IS NULL OR adm2.id=_scope_recipient)
    AND (_scope_ecosystem IS NULL OR ro.ecosystem_id=_scope_ecosystem)

  UNION ALL

  /* Platform credit supply — Super Admin only, never shop earnings. */
  SELECT
    'cg:' || cl.id::text,
    cl.created_at,
    cl.ecosystem_id,
  ...
  WHERE _super AND cl.created_at >= _start AND cl.created_at <= _end
    ...
    AND NOT EXISTS (
      ...
      WHERE regexp_replace(o.tx_id,'-R$','')=regexp_replace(cl.tx_id,'-R$','')
        AND o.id<>cl.id AND o.direction<>cl.direction
    )
  ...
  /* Platform subscription revenue — Super Admin only. */
  SELECT
    'ps:' || sr.id::text,
  ...
  WHERE _super AND _scope_recipient IS NULL AND sr.status='approved'
    AND COALESCE(sr.reviewed_at,sr.updated_at) >= _start AND COALESCE(sr.reviewed_at,sr.updated_at) <= _end
