-- Spending Tracker accounting alignment (reporting only, no money moves).
-- 1. Admin self-purchases are counted as shop sales income.
-- 2. Platform fee on every voucher sale is an expense (income grossed up for
--    the commission rows that were recorded net of the fee).
-- 3. Reward points actually awarded are an expense at 1 point = 1 coin,
--    replacing the historical reconciliation ledger rows so the same cost is
--    never counted twice.
...
BEGIN
  ...
  -- Admin self-purchase: a real sale of the shop's own stock. Income is the
  -- collected price less any downline cashback on that sale (i.e. before the
  -- platform fee and before the reward point cost, both expensed below).
  SELECT
    'sp:' || s.id::text,
    'income',
    s.created_at,
    ...
    ROUND(s.sale_price - COALESCE((
      SELECT SUM(sc.commission_amount) FROM public.sale_commissions sc
      WHERE sc.sale_id = s.id AND sc.kind <> 'admin' AND sc.reversed_at IS NULL), 0), 2),
    'admin_self_purchase',
    NULL::uuid,
    NULL::text
  FROM self_sale s
  WHERE ROUND(s.sale_price - COALESCE((
      SELECT SUM(sc.commission_amount) FROM public.sale_commissions sc
      WHERE sc.sale_id = s.id AND sc.kind <> 'admin' AND sc.reversed_at IS NULL), 0), 2) <> 0
  ...
  -- Reversed awards are excluded; no row exists at all while reward points are
  -- disabled, so a disabled shop is never charged.
  SELECT
    'pt:' || pl.id::text,
    'expense',
    pl.created_at,
