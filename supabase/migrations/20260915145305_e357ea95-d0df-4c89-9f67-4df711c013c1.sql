-- Resolve the shop's settling admin exactly as purchase_voucher does.
create or replace function public.shop_primary_admin(_eco uuid)
returns uuid
language plpgsql
stable
security definer
...
declare _admin uuid;
begin
  if _eco is null then return null; end if;
  select ur.user_id into _admin
    from public.user_roles ur
    join public.profiles pr on pr.id = ur.user_id
   where ur.ecosystem_id = _eco and ur.role = 'admin'
     and pr.deleted_at is null and pr.status = 'active'
   order by pr.joined_at
   limit 1;
  if _admin is null then
    select m.user_id into _admin
      from public.ecosystem_memberships m
      join public.profiles pr on pr.id = m.user_id
     where m.ecosystem_id = _eco and m.role = 'admin' and m.membership_state = 'active'
       and pr.deleted_at is null and pr.status = 'active'
     order by pr.joined_at
     limit 1;
  end if;
...
revoke all on function public.shop_primary_admin(uuid) from public;
grant execute on function public.shop_primary_admin(uuid) to authenticated, service_role;

-- Admin self-purchase netting: the admin's own shop remainder is applied at
-- checkout, and the coin cost of the points the purchase earns (1 pt = 1 coin)
-- is withheld from that remainder.
create or replace function public.voucher_admin_self_net(
  _total numeric, _fee numeric, _other numeric, _ratio numeric)
returns table(self_cashback numeric, buyer_charge numeric, points_earned numeric, admin_benefit numeric)
language plpgsql
immutable
set search_path to 'public'
as $$
declare _g numeric(14,2) := round(coalesce(_total,0),2);
        _benefit numeric(14,2);
        _basis numeric(14,2);
        _pts numeric(14,2) := 0;
        _cb numeric(14,2);
begin
  _benefit := greatest(round(_g - round(coalesce(_fee,0),2) - round(coalesce(_other,0),2), 2), 0);
  _basis := round(_g - _benefit, 2);
  if coalesce(_ratio,0) > 0 then _pts := round(_basis / _ratio, 2); end if;
  _cb := greatest(round(_benefit - _pts, 2), 0);
  return query select _cb, round(_g - _cb, 2), _pts, _benefit;
end $$;

revoke all on function public.voucher_admin_self_net(numeric, numeric, numeric, numeric) from public;
grant execute on function public.voucher_admin_self_net(numeric, numeric, numeric, numeric) to authenticated, service_role;
...
declare _subject uuid; _op uuid; _my_eco uuid; _p public.voucher_products; _acct uuid; _pacct uuid;
        ...
        _admin_self boolean := false; _other numeric(14,2) := 0; _points_basis numeric(14,2);
...
begin
  ...
  if _total > 0 then
    ...
    elsif _universe and _role = 'admin'
          and not public.is_super_admin(_subject)
          and public.shop_primary_admin(_my_eco) = _subject then
      -- The admin's own shop remainder is applied UP FRONT (same net-charge
      -- mechanism resellers use), minus the coin cost of the points earned.
      _admin_self := true;
      if _sale_reseller is not null then
        select coalesce(sum(round(_total * s.pct / 100.0, 2)), 0) into _other
          from public.cashback_chain(_sale_reseller, _my_eco) s;
      end if;
      select n.self_cashback, n.buyer_charge, n.points_earned, n.admin_benefit
        into _self_cb, _charge, _pt_cost, _benefit
        from public.voucher_admin_self_net(_total, _fee, _other,
               case when _pacct is null then 0 else coalesce(_ratio,0) end) n;
      _points_basis := round(_total - _benefit, 2);
      _self := _self_cb > 0;
      _own_pct := case when _total > 0 then round(_self_cb * 100.0 / _total)::int else 0 end;
    end if;
    ...
    if coalesce(_ratio,0) > 0 then
      _earn := round(greatest(coalesce(_points_basis, _charge, _total), 0) / _ratio, 2);
      if _pacct is null then _earn := 0; end if;
  ...
  if _self_cb > 0 and not _admin_self then
  ...
  if _debit is not null and _total > 0 then
    _admin_id := public.shop_primary_admin(_my_eco);
  ...
  if _admin_self then
    -- The admin's remainder was already applied inside the purchase debit:
    -- the commission row stays for reports but is never credited again.
    update public.sale_commissions sc set ledger_id = _debit
     where sc.sale_id = _sale and sc.recipient_id = _subject
       and sc.kind = 'admin' and sc.ledger_id is null;
  end if;
  ...
  if _earn > 0 and _pacct is not null then
    ...
    values (_pacct, _subject, _my_eco, 'credit', _earn, 0,
            ...
            _tx, _op, _tx || '-P', 'earn', _sale, coalesce(_points_basis, _charge, _total), _ratio, _ver);
  end if;
  ...
  perform public.log_operator_action(_subject, _my_eco, 'Voucher purchase', 'voucher_sale', _sale,
    jsonb_build_object('product', _p.name, 'quantity', _qty, 'unit_price', _unit, 'total', _total, 'tx_id', _tx,
                       ...
                       'admin_self_purchase', _admin_self, 'admin_point_coin_cost', _pt_cost));
...
-- Checkout quote now also reports the platform fee and the points the purchase earns.
drop function if exists public.voucher_checkout_quote(uuid, integer, uuid);

CREATE OR REPLACE FUNCTION public.voucher_checkout_quote(_product_id uuid, _quantity integer DEFAULT 1, _seller_id uuid DEFAULT NULL::uuid)
 RETURNS TABLE(total numeric, self_cashback numeric, buyer_charge numeric, self_purchase boolean, cashback_percent integer, platform_fee numeric, points_earned numeric)
 LANGUAGE plpgsql
...
declare _uid uuid := public.effective_uid(); _p record; _eco uuid; _role public.app_role; _discount int := 0;
        ...
        _fee numeric(14,2) := 0; _other numeric(14,2) := 0; _ratio numeric := 0;
        _pts numeric(14,2) := 0; _benefit numeric(14,2) := 0; _universe boolean := false;
begin
  if _uid is null then return query select 0::numeric,0::numeric,0::numeric,false,0,0::numeric,0::numeric; return; end if;
  select * into _p from public.voucher_products where id = _product_id;
  if _p is null then return query select 0::numeric,0::numeric,0::numeric,false,0,0::numeric,0::numeric; return; end if;
  _eco := _p.ecosystem_id;
  _universe := public.is_universe_shop(_eco);
  ...
  _total := round(_unit * _qty, 2);
  _charge := _total;
  select credits_per_point into _ratio from public.ecosystems where id = _eco;
  if _universe and _total > 0 then
    _fee := public.voucher_platform_fee_amount(_total, round(coalesce(_p.platform_fee_percent,0),2));
  end if;
  ...
  if _universe and _role in ('reseller','subreseller') then
    ...
    if coalesce(_ratio,0) > 0 then _pts := round(greatest(_charge,0) / _ratio, 2); end if;
  elsif _universe and _total > 0 and _role = 'admin'
        and not public.is_super_admin(_uid)
        and public.shop_primary_admin(_eco) = _uid then
    if _seller_id is not null and _seller_id <> _uid then
      select coalesce(sum(round(_total * s.pct / 100.0, 2)), 0) into _other
        from public.cashback_chain(_seller_id, _eco) s;
    end if;
    select n.self_cashback, n.buyer_charge, n.points_earned, n.admin_benefit
      into _cb, _charge, _pts, _benefit
      from public.voucher_admin_self_net(_total, _fee, _other, coalesce(_ratio,0)) n;
    _self := _cb > 0;
    _pct := case when _total > 0 then round(_cb * 100.0 / _total)::int else 0 end;
  else
    if coalesce(_ratio,0) > 0 then _pts := round(greatest(_total,0) / _ratio, 2); end if;
  end if;

  return query select _total, _cb, _charge, _self, case when _self then _pct else 0 end, _fee, _pts;
end $function$;

revoke all on function public.voucher_checkout_quote(uuid, integer, uuid) from public;
