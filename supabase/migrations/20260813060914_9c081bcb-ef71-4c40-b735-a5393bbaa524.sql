alter table public.platform_settings
  add column if not exists admin_voucher_discount_percent integer not null default 100;

alter table public.platform_settings
  drop constraint if exists platform_settings_admin_voucher_discount_check;
alter table public.platform_settings
  add constraint platform_settings_admin_voucher_discount_check
  check (admin_voucher_discount_percent between 0 and 100);

create or replace function public.admin_voucher_discount_percent()
returns integer
language sql
stable
security definer
...
as $$
  select least(greatest(coalesce((select admin_voucher_discount_percent from public.platform_settings where id = 1), 100), 0), 100)
$$;

grant execute on function public.admin_voucher_discount_percent() to authenticated;
...
begin
  ...
  -- Shop admins buy from their own uploaded inventory at the platform-configured
  -- admin voucher shop discount (default 100% off). This is separate from the
  -- admin credit allocation base rate.
...
declare _my_eco uuid; _p public.voucher_products; _acct uuid; _pacct uuid;
        ...
        _admrate integer := 0; _admin_id uuid;
begin
  ...
  -- A fully discounted purchase costs nothing, so there is no wallet movement to
  -- record. The sale row above still carries the list price, discount and zero cost.
  if _total > 0 then
  ...
  -- Shop admin commission on member purchases (never on the admin's own purchase).
  _admrate := public.admin_sale_commission_rate_for(_my_eco);
  if _admrate > 0 and _debit is not null then
    select ur.user_id into _admin_id
     ...
     limit 1;
    if _admin_id is not null and _admin_id <> auth.uid() then
      _amt := round(_total * _admrate / 100.0, 2);
      if _amt > 0 then
        insert into public.sale_commissions (ecosystem_id, sale_id, recipient_id, source_lot_id,
                                             source_ledger_id, credits_consumed, commission_percent,
                                             commission_amount, kind)
  ...
  loop
    ...
    values (_racct, _rec.recipient_id, _my_eco, 'credit', _rec.amount, 0,
            case _rec.kind
                 ...
                 when 'admin'
                   then 'Shop commission — ' || _p.name || ' ×' || _qty || ' (' || _rec.pct || '% of a member purchase)'
