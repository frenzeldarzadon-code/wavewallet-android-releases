...
begin
  ...
  -- Everyone whose shop-scoped records belong to this shop: the legacy home
  -- pointer plus anyone holding a membership in it.
  select coalesce(array_agg(distinct s.u), '{}')
    into _members
    from (
      select p.id as u from public.profiles p where p.ecosystem_id = _ecosystem_id
      union
      select m.user_id from public.ecosystem_memberships m where m.ecosystem_id = _ecosystem_id
    ) s
   where not public.is_super_admin(s.u);
  ...
  -- Shop-scoped role links only. Membership rows cascade with the shop below.
  ...
  -- The global Universe identity survives the shop: profiles are never deleted
  -- here. Shop pointers move to a surviving membership when one exists,
  -- otherwise they are cleared and the member stays Universe-only.
  update public.profiles p
     set ecosystem_id = case
           when p.ecosystem_id = _ecosystem_id then (
             select m.ecosystem_id from public.ecosystem_memberships m
              where m.user_id = p.id and m.ecosystem_id <> _ecosystem_id
              order by m.joined_at nulls last, m.created_at limit 1)
           else p.ecosystem_id end,
         active_ecosystem_id = case
           when p.active_ecosystem_id = _ecosystem_id then (
             select m.ecosystem_id from public.ecosystem_memberships m
              where m.user_id = p.id and m.ecosystem_id <> _ecosystem_id
              order by m.joined_at nulls last, m.created_at limit 1)
           else p.active_ecosystem_id end
   where p.ecosystem_id = _ecosystem_id or p.active_ecosystem_id = _ecosystem_id;
