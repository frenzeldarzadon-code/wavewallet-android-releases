...
BEGIN
  ...
  -- Shop (admin) retained share of a completed voucher sale for LEGACY sales
  -- that carry no explicit admin remainder row: the credits collected, less
  -- the reseller/subreseller cashback allocated to that same sale.
  SELECT
    'am:' || vs.id::text,
    vs.created_at,
    vs.ecosystem_id,
  ...
  WHERE vs.created_at >= _start AND vs.created_at <= _end
    ...
    -- Modern sales already carry a real `admin` sale_commissions row reported
    -- above; deriving a second margin row here would double count.
    AND NOT EXISTS (
  ...
  -- value once the order is approved.
  SELECT
    'ro:' || ro.id::text,
  ...
  -- Platform-minted credit supply. Platform owner only; never shop earnings.
  SELECT
    'cg:' || cl.id::text,
    cl.created_at,
    cl.ecosystem_id,
