...
BEGIN
  ...
  -- allocated to that same sale (rates snapshotted at sale time). Refunded
  -- sales keep their row but are marked reversed; points-funded sales collect
  -- no credits and never produce shop earnings.
  SELECT
    'am:' || vs.id::text,
    vs.created_at,
    vs.ecosystem_id,
  ...
  LEFT JOIN LATERAL (
    ...
    WHERE sc2.sale_id = vs.id AND sc2.reversed_at IS NULL
  ) c ON true
  LEFT JOIN LATERAL (
    SELECT p.id, p.full_name
    FROM public.profiles p
    JOIN public.user_roles ur ON ur.user_id = p.id AND ur.role = 'admin'
    WHERE p.ecosystem_id = vs.ecosystem_id
