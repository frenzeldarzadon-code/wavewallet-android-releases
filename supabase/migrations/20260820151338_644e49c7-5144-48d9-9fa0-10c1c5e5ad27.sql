...
declare
  ...
  _actor text; _target text; _role app_role; _op uuid;
...
begin
  ...
  _op := coalesce(auth.uid(), (select r.user_id from public.user_roles r
                                where r.role = 'super_admin' order by r.created_at limit 1));
  if _op is null then raise exception 'No platform owner is configured'; end if;
  ...
  values (_acct, _user_id, _eco, 'credit', _amount, 0, trim(_reason),
          nullif(trim(_reference),''), _op, _tx, 'superadmin_credit_issuance',
          _amount, 0, 0)
  returning id, balance_after into _ledger, _after;

  select full_name into _actor from public.profiles where id = _op;
  ...
  values (_tx, _key, _op, coalesce(_actor,'WaveWallet platform'), _user_id, _target,
  ...
  values (_eco, _op, coalesce(_actor,'WaveWallet platform'),
          ...
          jsonb_build_object('amount', _amount, 'reason', trim(_reason),
                             ...
                             'operator_id', _op, 'action_type', 'manual_credit',
