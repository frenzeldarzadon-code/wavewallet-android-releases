...
as $$
  select p.id, p.full_name, p.handle, p.avatar_path
   ...
   where auth.uid() is not null
     ...
     and coalesce(p.is_demo, false) = coalesce((select me.is_demo from public.profiles me where me.id = public.effective_uid()), false)
...
declare
  ...
  _recipient_status public.account_status; _deleted timestamptz; _demo boolean; _sender_demo boolean;
...
begin
  ...
  select p.full_name, p.handle, coalesce(p.is_demo,false)
    into _sender_name, _sender_handle, _sender_demo
  ...
  if _target is null or _deleted is not null
     or not exists (select 1 from auth.users u where u.id = _recipient_id) then
  ...
  -- Demo isolation: demo coins stay between demo accounts; real coins never reach a demo account.
  if _demo <> _sender_demo then
    raise exception 'Demo coins can only move between demo accounts';
  end if;
  ...
  values(null,_op,coalesce(_actor_name,'Member'),'Transferred Universe coins',coalesce(_target,''),
         jsonb_build_object('amount',_amount,'commission_percent',0,'commission_amount',0,'total_received',_amount,
                            'tx_id',_tx,'global_wallet',true,'sender_id',_subject,'recipient_id',_recipient_id,
                            'client_key',_key,'demo',_sender_demo));
  return _tx;
end;
$$;