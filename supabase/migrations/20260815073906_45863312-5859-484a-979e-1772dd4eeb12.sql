-- ===================================================================
-- Authoritative cashback model, universal to every shop.
-- ===================================================================

create or replace function public.cashback_chain(_source uuid, _ecosystem_id uuid)
returns table(recipient_id uuid, kind text, pct integer)
language plpgsql
stable security definer
set search_path to 'public'
as $function$
declare _role public.app_role; _parent uuid; _own integer; _ptotal integer;
begin
  if _source is null or _ecosystem_id is null then return; end if;
  if public.is_super_admin(_source) then return; end if;

  select m.role, coalesce(m.reseller_id, p.reseller_id)
    into _role, _parent
    from public.profiles p
    left join public.ecosystem_memberships m
           on m.user_id = p.id and m.ecosystem_id = _ecosystem_id
   where p.id = _source;

  if _role is null then
    select ur.role into _role from public.user_roles ur
     where ur.user_id = _source and ur.ecosystem_id = _ecosystem_id
     order by case ur.role when 'reseller' then 0 when 'subreseller' then 1 else 2 end
     limit 1;
  end if;
  if _role is null or _role not in ('reseller','subreseller') then return; end if;

  _own := coalesce(public.member_cashback_rate(_source, _ecosystem_id), 0);

  if _role = 'subreseller' and _parent is not null then
    _ptotal := coalesce(public.member_cashback_rate(_parent, _ecosystem_id), 0);
    _own := least(_own, _ptotal);
    if _own > 0 then
      recipient_id := _source; kind := 'sale_cashback'; pct := _own; return next;
    end if;
    if _ptotal - _own > 0 then
      recipient_id := _parent; kind := 'upline'; pct := _ptotal - _own; return next;
    end if;
  else
    if _own > 0 then
      recipient_id := _source; kind := 'sale_cashback'; pct := _own; return next;
...
revoke all on function public.cashback_chain(uuid, uuid) from public;
grant execute on function public.cashback_chain(uuid, uuid) to authenticated, service_role;

create or replace function public.cashback_split_preview(_user_id uuid, _ecosystem_id uuid)
returns jsonb
language plpgsql
stable security definer
set search_path to 'public'
as $function$
declare _role public.app_role; _parent uuid; _own integer; _ptotal integer; _sub integer := 0;
begin
  select m.role, coalesce(m.reseller_id, p.reseller_id) into _role, _parent
    from public.profiles p
    left join public.ecosystem_memberships m
           on m.user_id = p.id and m.ecosystem_id = _ecosystem_id
   where p.id = _user_id;
  ...
  _own := coalesce(public.member_cashback_rate(_user_id, _ecosystem_id), 0);
  if _role = 'subreseller' then
    _ptotal := coalesce(public.member_cashback_rate(_parent, _ecosystem_id), 0);
    return jsonb_build_object('role','subreseller','own',least(_own,_ptotal),
      'parent_id',_parent,'parent_total',_ptotal,
      'parent_share',greatest(_ptotal - least(_own,_ptotal),0),
      'admin_share',greatest(100 - _ptotal,0));
  end if;
  select coalesce(max(public.member_cashback_rate(m.user_id, _ecosystem_id)),0) into _sub
    from public.ecosystem_memberships m
   where m.ecosystem_id = _ecosystem_id and m.role = 'subreseller' and m.reseller_id = _user_id;
  return jsonb_build_object('role', coalesce(_role::text,'customer'), 'own', _own,
    'parent_id', null, 'parent_total', _own,
    'parent_share', greatest(_own - _sub, 0),
    'max_subreseller', _sub,
    'admin_share', greatest(100 - _own, 0));
end $function$;

revoke all on function public.cashback_split_preview(uuid, uuid) from public;
grant execute on function public.cashback_split_preview(uuid, uuid) to authenticated, service_role;
...
begin
  ...
  if _role = 'subreseller' then
    ...
    if _percent > _other then
      raise exception 'A subreseller share comes out of the parent reseller share (parent total is %%%)', _other;
    end if;
  else
    ...
    if _percent < coalesce(_other, 0) then
      raise exception 'A subreseller is set to %%% — the reseller total cannot be lower', _other;
    end if;
  end if;
