-- Duplicate-checkout protection (additive): one order per client checkout attempt.
alter table public.retail_orders add column if not exists client_ref uuid;
create unique index if not exists retail_orders_customer_client_ref_key
  on public.retail_orders (customer_id, client_ref) where client_ref is not null;

drop function if exists public.retail_place_order(uuid, jsonb, text, text, text, text, uuid);
...
begin
  ...
  if _client_ref is not null then
    select o.id, o.order_no, o.total into _oid, _ono, _total
      from public.retail_orders o where o.customer_id = _uid and o.client_ref = _client_ref;
    if _oid is not null then return query select _oid, _ono, _total; return; end if;
  end if;
