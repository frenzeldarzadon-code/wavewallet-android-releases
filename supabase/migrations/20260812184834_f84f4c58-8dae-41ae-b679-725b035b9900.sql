create table if not exists public.test_data_resets (
...
grant select on public.test_data_resets to authenticated;
grant all on public.test_data_resets to service_role;

alter table public.test_data_resets enable row level security;

create policy "Platform owner reads test data resets"
  on public.test_data_resets for select to authenticated
...
begin
  ...
  -- return sold test voucher codes to inventory
  update public.voucher_codes
     set status = 'unused', sold_to = null, sale_id = null, sold_at = null
   where ecosystem_id = _ecosystem_id and status <> 'unused';

  -- restore reward stock consumed by test redemptions
