-- 1. Lock internal SECURITY DEFINER helpers away from anonymous callers.
--    Public sign-up + first-owner bootstrap helpers intentionally stay open.
revoke execute on function public.countable_members(uuid) from anon, authenticated;
revoke execute on function public.ecosystem_monthly_rate(uuid) from anon;
revoke execute on function public.ecosystem_last_activity(uuid) from anon;
revoke execute on function public.ecosystem_cleanup_check(uuid) from anon;
revoke execute on function public.customer_deletion_check(uuid) from anon;
revoke execute on function public.upline_commission_rate_for(uuid) from anon;
revoke execute on function public.voucher_discount_percent_for(uuid) from anon;
revoke execute on function public.platform_overview() from anon;
revoke execute on function public.ecosystem_dashboard(uuid) from anon;
revoke execute on function public.earnings_history(uuid, uuid, timestamptz, timestamptz) from anon;
revoke execute on function public.adjust_ecosystem_expiration(uuid, timestamptz, text, text, boolean) from anon;
revoke execute on function public.archive_ecosystem(uuid, text) from anon;
revoke execute on function public.delete_customer_account(uuid, text) from anon;
revoke execute on function public.run_ecosystem_cleanup(boolean) from anon;
revoke execute on function public.set_ecosystem_rates(uuid, integer, integer, integer, integer, integer) from anon;

-- 2. Points recorded on a sale must always match the points ledger.
...
begin
  ...
  -- Resolve the points wallet BEFORE the sale is written: without a wallet no
  -- points ledger entry can exist, so the sale must not claim any either.
  if _earn > 0 then
...
-- 3. Platform-owner wallet integrity report.
--    A wallet may legitimately differ from its ledger sum once the 12-month
--    retention purge has removed old entries, so purged accounts are reported
--    separately instead of being flagged as broken.
create or replace function public.wallet_integrity_check()
 returns table(kind text, account_id uuid, user_id uuid, ecosystem_id uuid,
               member_name text, balance numeric, ledger_sum numeric, difference numeric,
               oldest_entry timestamptz, purge_explained boolean)
 language plpgsql
 stable
 security definer
 set search_path to 'public'
...
begin
  ...
  select max(cutoff) into _last_purge
    from public.retention_runs where dry_run = false and status = 'success';
  ...
  select 'credits'::text, c.id, c.user_id, c.ecosystem_id, pr.full_name,
         c.balance, c.summed, c.balance - c.summed, c.oldest,
  ...
  union all
  select 'points'::text, p.id, p.user_id, p.ecosystem_id, pr.full_name,
         p.balance, p.summed, p.balance - p.summed, p.oldest,
...
revoke execute on function public.wallet_integrity_check() from anon;
grant execute on function public.wallet_integrity_check() to authenticated;