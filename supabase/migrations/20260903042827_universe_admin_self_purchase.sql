-- Admin self-purchases in Universe shops: the admin pays the retail price they
-- set (no admin purchase discount); the shop remainder still flows to the admin.
-- New Generation shops keep the platform admin discount rule unchanged.
...
begin
  ...
  if _debit is not null and _total > 0 then
    ...
    -- Universe: the shop's remainder (the seller's cut) is always paid to the
    -- shop admin — including when the admin is the buyer, who now pays the
    -- same retail price as everyone. Otherwise that cut would simply vanish.
    -- New Generation keeps the original rule (admin self-purchases cost 0).
