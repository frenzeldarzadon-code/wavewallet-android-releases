...
export function WalletCenter({ base, showSellerTotals = false, scope = "shop" }: WalletCenterProps) {
  ...
  const loadShops = useCallback(async () => {
    ...
    if (universe && userId) {
      ...
      list = all
        .filter((_, i) => views[i]?.isGlobal === true)
        .map((s) => ({ ...s, balance }));
    }
    setShops(list);
    setSelectedId((cur) =>
      universe
        ? (cur && list.some((s) => s.ecosystemId === cur) ? cur : (list[0]?.ecosystemId ?? null))
