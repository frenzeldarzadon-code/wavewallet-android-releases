...
export function manualCreditReason(note?: string | null): string {
  const extra = (note ?? "").trim();
  return extra ? `${MANUAL_CREDIT_ACTION} — ${extra}` : MANUAL_CREDIT_ACTION;
}
...
export async function grantManualCredit(input: {
  ...
  return adminAdjustCredits({
    userId: input.userId,
    amount: input.amount,
    reason: manualCreditReason(input.note ?? null),
